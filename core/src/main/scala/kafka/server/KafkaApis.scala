...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handleProducerRequest(request: RequestChannel.Request) {
    ...
    //todo: 按照分区的方式进行遍历
    //todo: 由于在封装请求的过程中，为了提升效率会把发送给同一台主机的多个分区数据封装为一个请求，so 这里会出现一个请求中有多个分区数据
    //existingAndAuthorizedForDescribeTopics: 主题存在，有权限，正常的情况
    //nonExistingOrUnauthorizedForDescribeTopics: 主题不存在或者没有权限，不正常的情况
    ...
    def sendResponseCallback(responseStatus: Map[TopicPartition, PartitionResponse]) {
      ...
      def produceResponseCallback(delayTimeMs: Int) {
        ...
        if (produceRequest.acks == 0) {
          ...
          //todo: 返回响应。封装Response对象，它就是服务端返回给客户端的
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    // the callback for sending a fetch response    todo: 定义回调函数返回响应
    ...
    //todo:如果没有一个topic通过上面的筛选，就发送响应给客户端
    if (authorizedRequestInfo.isEmpty)
      sendResponseCallback(Seq.empty)
    else {
      // call the replica manager to fetch messages from the local replica
      //todo:调用replicaManager的fetchMessages方法拉取消息
      replicaManager.fetchMessages(
        ...
        sendResponseCallback)  //最后调用了回调函数
    }
  }
