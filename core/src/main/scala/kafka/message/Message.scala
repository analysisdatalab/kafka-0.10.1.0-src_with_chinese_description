 ...
 *
 * 1. 4 byte CRC32 of the message      用来校验数据
 * 2. 1 byte "magic" identifier to allow format changes, value is 0 or 1
  *         0 表示绝对的offset
  *         1 表示相对的offset
 * 3. 1 byte "attributes" identifier to allow annotations on the message independent of the version
  *     1字节有八位，不同的位置表示消息不同的属性
 *    bit 0 ~ 2 : Compression codec.      0-2位：是否支持压缩
 *      0 : no compression
 *      1 : gzip
 *      2 : snappy
 *      3 : lz4
 *    bit 3 : Timestamp type     第三位表示：时间类型，
 *      0 : create time           0表示创建时间
 *      1 : log append time      1表示日志追加时间
 *    bit 4 ~ 7 : reserved    第4-7位是预留，方便以后记录其他信息
