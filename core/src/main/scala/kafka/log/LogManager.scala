...
class LogManager(val logDirs: Array[File],
  ...
  //todo: 核心参数
   // 结论： 一个分区（磁盘上面的目录）对应一个Log对象        TopicAndPartition(topic: String, partition: Int)
  ...
  private def createAndValidateLogDirs(dirs: Seq[File]) {
    ...
    //todo: 遍历目录然后创建
    //todo: log.dirs=/data0/log, /data1/log,/data2/log
    for(dir <- dirs) {
      if(!dir.exists) {
  ...
  private def loadLogs(): Unit = {
    ...
    for (dir <- this.logDirs) {
      //todo: 为每一个目录创建一个线程池，后面启动线程池中的线程去加载log
      ...
      val jobsForDir = for {
        //todo:获取每一个目录下的所有文件和目录
        dirContent <- Option(dir.listFiles).toList
       //todo: 获取每一个分区目录logDir
  ...
  def startup() {
    ...
    if(scheduler != null) {
      ...
      scheduler.schedule("kafka-log-retention",
                         ...
                         period = retentionCheckMs,  //默认是5分钟
                         TimeUnit.MILLISECONDS)
      info("Starting log flusher with a default period of %d ms.".format(flushCheckMs))
      //todo:（2） 定时把内存中的数据刷到磁盘中
      scheduler.schedule("kafka-log-flusher", 
                         flushDirtyLogs, 
                         delay = InitialTaskDelayMs, 
                         period = flushCheckMs, 
                         TimeUnit.MILLISECONDS)
      //todo:（3） 定时更新检查点文件，kafka重启的时候恢复数据用的   ，定期的内存中的数据更新到该kafka-recovery-point-checkpoint文件中
      //这一块不是我们主要要分析的，了解即可
  ...
  private def flushDirtyLogs() = {
    ...
    //todo: 遍历logs 获取每一个分区对应的Log文件
    for ((topicAndPartition, log) <- logs) {
      try {
        val timeSinceLastFlush = time.milliseconds - log.lastFlushTime
        debug("Checking if flush is needed on " + topicAndPartition.topic + " flush interval  " + log.config.flushMs +
              " last flushed " + log.lastFlushTime + " time since last flush: " + timeSinceLastFlush)
        //todo: 按照一定的频率把数据刷到磁盘中，  这个频率的阈值是Long类型的最大值
