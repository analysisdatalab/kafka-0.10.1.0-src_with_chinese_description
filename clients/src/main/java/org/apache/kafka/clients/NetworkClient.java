...
public class NetworkClient implements KafkaClient {
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        ...
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            //todo:  从InFlightRequests中移除掉已经接受到响应的请求
            ClientRequest req = inFlightRequests.completeNext(source);
            //解析响应,数据封装成Struct对象中
            Struct body = parseResponse(receive.payload(), req.request().header());

            //todo: maybeHandleCompletedReceive 如果是关于元数据的响应
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        private void handleResponse(RequestHeader header, Struct body, long now) {
            this.metadataFetchInProgress = false;
            //todo: 解析响应的数据Struct对象，封装成MetadataResponse
            ...
            if (cluster.nodes().size() > 0) {
                ...
                //todo: 更新元数据信息，接下来进入到了我们之前分析过得update方法
        ...
        private void maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId)) {
                ...
                if (metadata.needMetadataForAllTopics())
                    //todo:获取所有topic的元数据信息
                    //todo:但是我们一般获取元数据的时候，只获取自己要发送消息的topic的元数据信息
